...
Actual data: http://py4e-data.dr-chuck.net/comments_2143398.xml (Sum ends with 32)
...
xmldata = urllib.request.urlopen(url).read() #reads the XML data from the web, decodes and stores it into variable xmldata
tree = ET.fromstring(xmldata)
counts = tree.findall('.//count')
print("Count: ", len(counts))
#or can also loop through the nodes, both parent & child to find comments.
sum = 0
for item in counts:
